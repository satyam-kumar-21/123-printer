...
// 1. Modified to take modelsearchinput as an argument instead of reading localStorage immediately
function getDefaultSteps(modelsearchinput) {
    return [
        {
...
export default function SetupProgressModal({ open, onClose, printer = '', user = '', onError }) {
    ...
    // 2. Safely initialize state. In Next.js, this avoids running localStorage on the server.
    const [stepStates, setStepStates] = useState(() => {
        let model = '';
        if (typeof window !== 'undefined') {
            model = localStorage.getItem('printerModel') || '';
        }
        return getDefaultSteps(model);
    });
    ...
    useEffect(() => {
        ...
        // 3. Safely fetch the updated value whenever the modal opens on the client side
        const model = typeof window !== 'undefined' ? localStorage.getItem('printerModel') : '';
        const freshSteps = getDefaultSteps(model);
        ...
        function animateStep(idx) {
            ...
            const interval = setInterval(() => {
                ...
                if (idx === 2 && prog >= 60) {
                    ...
                    if (idx < 2) { // dynamic steps length replacement check
